...
- Write-tool payloads carry input-size caps (file content, commit/PR/issue/comment text, and per-commit file count — see `src/tools/common.ts`) as defence-in-depth, so a runaway model can't burn Worker CPU/memory with a multi-megabyte string well under the platform's 100 MiB request limit. Oversized input is rejected at schema validation with a descriptive error.
