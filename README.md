...
Several GitHub MCP servers exist, alongside GitHub's own `gh` CLI. This server's niche: responses are **curated for an LLM consumer** — issue and PR detail tools return complete bodies, file reads support bounded ranges, and other tools curate or truncate large payloads — and every call runs under the user's own GitHub OAuth identity. (Output is serialized as Markdown rather than JSON; that is a deliberate format trade-off, not a strict win — see **Differentiators** below.)
...
| Issue read / triage / lifecycle (labels, assignees, comments)                 | 🟡 sub-issues ([#161](https://github.com/nemolize/remote-mcp-github/issues/161))                                                                                                                               | ✅                        | ✅                                                             |
...
| Actions administration (secrets, variables, cache, workflow enable / disable) | 🟡 Codespaces / Dependabot secrets ([#202](https://github.com/nemolize/remote-mcp-github/issues/202))                                                                                                          | ❌                        | ✅                                                             |
...
| Label definitions (create / edit / delete / clone)                            | ✅                                                                                                                                                                                                             | 🟡 read only              | ✅                                                             |
...
| Attachment upload (images / video for issue & PR bodies)                      | ✅ `upload_attachment` — the nine types GitHub's endpoint accepts; archives and documents are refused server-side                                                                                              | ❌                        | 🟡 `gh --attach` (2.99+, media only)                           |
...
- **Context-bounded, curated output.** Issue and PR detail tools return complete bodies. File reads return bounded chunks with a continuation offset; other tools curate or truncate large payloads to reduce context use. Output is serialized as Markdown — a deliberate format trade-off: denser and closer to how the model consumes the result, at the cost of the unambiguous structure raw JSON gives a programmatic caller. The official server returns JSON.
...
All tools respond in Markdown (not raw JSON). `get_issue` and `get_pull_request` always return the complete body, without an expansion flag. `get_file_content` returns up to 8,000 Unicode code points of file text by default; use `offset` (zero-based) and `limit` (1–8,000) to select a range. Follow the next offset in the response to read the remainder, and pin `ref` to a commit SHA when reading multiple chunks of a changing file. These parameters apply only to files, not directory listings. File reads support up to 20,000,000 bytes (20 MB); decoding validates UTF-8 in small chunks and retains only the requested range. Each range request still fetches and validates the entire file. Binary files are rejected; write limits are unchanged.
...
| `list_issue_comments`              | read  | Conversation comments on an issue or PR (renders the `comment_id` for edit / delete)                 |
| `list_linked_branches`             | read  | Branches linked to an issue via the development sidebar (companion read for `develop_issue`)         |
...
| `create_label`                     | write | Create a label definition in the repo (name + optional colour / description)                         |
| `update_label`                     | write | Rename a label definition and / or change its colour / description                                   |
| `delete_label`                     | write | Delete a label definition from the repo (removes it everywhere it is applied; destructive)           |
| `clone_labels`                     | write | Copy label definitions from another repo (skips existing names, or `overwrite` to update them)       |
...
| `upload_attachment`                | write | Upload an image / video and return a `user-attachments` URL to embed (no archives; cannot be undone) |
| `update_issue_comment`             | write | Edit a conversation comment's body on an issue or PR (by `comment_id`)                               |
| `delete_issue_comment`             | write | Delete a conversation comment from an issue or PR (destructive)                                      |
| `pin_issue`                        | write | Pin an issue to the top of the repo's issue list (GraphQL; max three pinned per repo)                |
| `unpin_issue`                      | write | Unpin a pinned issue (GraphQL)                                                                       |
| `lock_issue`                       | write | Lock an issue / PR conversation (optional reason: off-topic / too heated / resolved / spam)          |
| `unlock_issue`                     | write | Unlock a previously locked issue / PR conversation                                                   |
| `transfer_issue`                   | write | Transfer an issue to another repo of the same owner (permanent move; GraphQL)                        |
| `delete_issue`                     | write | Permanently delete an issue (destructive, no restoration window; requires admin; GraphQL)            |
| `develop_issue`                    | write | Create a branch linked to an issue (GraphQL `createLinkedBranch`; optional name / base ref)          |
...
- Write-tool payloads carry input-size caps (file content, commit/PR/issue/comment text, per-commit file count, the aggregate content size of a multi-file commit — see `src/tools/common.ts` — and attachment bytes, capped to what the transport carries) as defence-in-depth, so a runaway model can't burn Worker CPU/memory with a multi-megabyte payload well under the platform's 100 MiB request limit. Oversized input is rejected with a descriptive error (per-field caps at schema validation; the aggregate-commit cap in the `commit_files` handler before any API call).
