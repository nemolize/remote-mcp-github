...
The table below compares coverage by **feature area** against the two most common alternatives. It stays deliberately coarse-grained — the [tool table](#whats-included) is the source of truth for which tools exist right now, and `gh`'s coverage is documented in the [GitHub CLI manual](https://cli.github.com/manual/). Legend: ✅ first-class · 🟡 partial — some tools in the area still missing (see the linked issue) · ⚠️ only via a lower-level escape hatch (`gh api`, local `git`) · ❌ absent.

| Feature area                                                  | This server                                                                                                                                                             | Official `mcp__github__*` | `gh` CLI                         |
| ------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------- | -------------------------------- |
| Repo metadata & discovery                                     | ✅                                                                                                                                                                      | ✅                        | ✅                               |
| Issue read / triage / lifecycle (labels, assignees, comments) | 🟡 core read / triage / lifecycle shipped; sub-issues not yet ([#161](https://github.com/nemolize/remote-mcp-github/issues/161))                                        | ✅                        | ✅                               |
| Code & repo search                                            | 🟡 repo / issue / code search shipped; users / orgs / PR search not yet ([#163](https://github.com/nemolize/remote-mcp-github/issues/163))                              | ✅                        | ✅                               |
...
| PR read detail / merge / update lifecycle                     | 🟡 core detail / merge / update shipped; per-file diff / mergeability status / branch update not yet ([#157](https://github.com/nemolize/remote-mcp-github/issues/157)) | ✅                        | ✅                               |
...
| Copilot delegation (assign issue / request PR review)         | ❌ ([#162](https://github.com/nemolize/remote-mcp-github/issues/162))                                                                                                   | ✅                        | ❌                               |
| Commit history (list / show / compare)                        | ✅                                                                                                                                                                      | ✅                        | ⚠️ `gh api` only                 |
| Workflow / Actions (CI status, logs, rerun)                   | 🟡 read + rerun / cancel / dispatch shipped; run usage / log deletion / artifact download not yet ([#159](https://github.com/nemolize/remote-mcp-github/issues/159))    | ✅                        | ✅                               |
...
| Security scanning (secret / code / Dependabot)                | 🟡 list-alert reads shipped; single-alert `get_*` not yet ([#158](https://github.com/nemolize/remote-mcp-github/issues/158))                                            | ✅                        | ⚠️ `gh api` only                 |
| Discussions (list / categories / get / comments)              | ❌ ([#160](https://github.com/nemolize/remote-mcp-github/issues/160))                                                                                                   | ✅                        | ⚠️ `gh api graphql` only         |
...
| Projects (v2)                                                 | ❌ ([#140](https://github.com/nemolize/remote-mcp-github/issues/140))                                                                                                   | ✅                        | ✅ (`gh project`)                |
...
- **PR review thread resolve / unresolve** ([#39](https://github.com/nemolize/remote-mcp-github/issues/39), shipped). The official MCP still cannot resolve a review thread, so its users fall back to `gh api graphql`; this server exposes it directly via `list_pr_review_threads` (discover thread IDs) + `resolve_review_thread` / `unresolve_review_thread`. This is the gap that motivated the table, and closing it is the clearest differentiator.
